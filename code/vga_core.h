...
public:
	...
	enum {
		...
		XY_BYPASS_REG = 0x0,
		COLOR_REG = 0x10,
		BYPASS_REG = 0x1000  /**< bypass control register */
	};
	/* methods */
	BlocksCore();
	BlocksCore(uint32_t core_base_addr3, uint32_t core_base_addr2, uint32_t core_base_addr1, uint32_t core_base_addr0);
	~BlocksCore();                  // not used
	...
	/**
	 * write which block you want to use, the x and y position, the bypass, and the color in that order
	 */
	void wr_blocks(uint32_t block, uint32_t x, uint32_t y, uint32_t block_bypass, uint32_t rgb);

	/**
	 * write which block you want to use, the x and y position, and the bypass in that order
	 */
	void wr_blocks_location_en(uint32_t block, uint32_t x, uint32_t y, uint32_t block_bypass);

	/**
	 * write which block you want to use and the x and y position in that order
	 */
	void wr_blocks_location(uint32_t block, uint32_t x, uint32_t y);

	/**
	 * write which block you want to use and the color in that order
	 */
	void wr_blocks_color(uint32_t block, uint32_t rgb);
